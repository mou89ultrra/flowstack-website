...
export default function WorkflowsPage() {
  ...
  const filtered = activeCategory === 'الكل' ? workflows : workflows.filter((w) => w.category === activeCategory)
  ...
  return (
    <>
      <main className="min-h-screen bg-[#0A0A0F] pt-24 pb-16">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((wf) => (
              <div key={wf.slug} className="card-hover rounded-2xl p-6 flex flex-col"
                style={{ background: 'linear-gradient(135deg, #111118, #1a1a2e)', border: `1px solid ${wf.color}25` }}>
                <div className="flex items-start justify-between mb-4">
                  <span className="text-xs px-2 py-0.5 rounded-full font-semibold"
                    style={{ background: `${wf.color}14`, color: wf.color }}>{wf.category}</span>
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {wf.tools.map((tool) => (
                    <span key={tool} className="text-xs px-2 py-0.5 rounded-md"
                      style={{ background: `${wf.color}10`, color: wf.color, border: `1px solid ${wf.color}25` }}>{tool}</span>
                  ))}
                </div>
                <div className="flex gap-2">
                  <button onClick={() => openModal(wf)}
                    ...
                    <Download size={14} />تحميل مجاناً
                  </button>
                  <a href={`/workflows/${wf.slug}`}
                    className="px-3 py-2.5 rounded-xl text-xs font-semibold"
                    style={{ background: `${wf.color}14`, color: wf.color, border: `1px solid ${wf.color}25` }}>تفاصيل</a>
                </div>
              </div>
            ))}
          </div>
      ...
      <Footer />
      <EmailModal isOpen={modalOpen} onClose={() => setModalOpen(false)} workflow={selectedWorkflow} />
    </>
  )
}
