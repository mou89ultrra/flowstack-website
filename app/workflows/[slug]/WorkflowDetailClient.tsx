  ...
  return (
    <>
        ...
        <div className="lg:col-span-2 space-y-8">
          <div>
            <span className="text-xs px-3 py-1 rounded-full font-semibold inline-block mb-4"
              style={{ background: `${workflow.color}14`, color: workflow.color }}>{workflow.category}</span>
          ...
          <div>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {workflow.useCases.map((uc) => (
                <div key={uc} className="flex items-start gap-2.5 p-3 rounded-xl text-sm text-gray-300"
              ...
              {workflow.steps.map((step, i) => (
                <div key={i} className="flex items-start gap-4">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-black shrink-0 text-white"
                    style={{ background: `linear-gradient(135deg, ${workflow.color}, ${workflow.color}99)` }}>{i + 1}</div>
        ...
        <div className="space-y-5">
          <div className="rounded-2xl p-6 sticky top-24"
            ...
            <button onClick={() => setModalOpen(true)}
              ...
              style={{ background: `linear-gradient(135deg, ${workflow.color}, ${workflow.color}bb)`, boxShadow: `0 8px 24px ${workflow.color}30` }}>
              <Download size={16} />تحميل مجاناً
            </button>

            <div className="space-y-2">
              <p className="text-xs text-gray-500 font-semibold">الأدوات المطلوبة:</p>
              {workflow.tools.map((tool) => (
                <div key={tool} className="flex items-center gap-2 text-xs text-gray-300 px-3 py-1.5 rounded-lg"
                  style={{ background: `${workflow.color}08`, border: `1px solid ${workflow.color}18` }}>
                  <span style={{ color: workflow.color }}>•</span>{tool}
                </div>
              ))}
            </div>
          </div>

          <a href={`https://wa.me/8201071994768?text=مرحباً، أريد مساعدة في إعداد workflow: ${workflow.title}`}
            target="_blank" rel="noopener noreferrer"
            className="block p-4 rounded-2xl text-center text-sm"
      ...
      {related.length > 0 && (
        <div className="mt-14">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {related.map((w) => (
              <a key={w.slug} href={`/workflows/${w.slug}`}
                ...
                <span className="text-xs px-2 py-0.5 rounded-full font-semibold inline-block mb-3"
                  style={{ background: `${w.color}14`, color: w.color }}>{w.category}</span>
      ...
      <EmailModal isOpen={modalOpen} onClose={() => setModalOpen(false)}
        workflow={{ title: workflow.title, file: workflow.file }} />
    </>
  )
}
